 ...
 * Native implementation of SQL Server DBO driver using the Microsoft SQL Server Driver for PHP v1.1
 *
 * @package       cake
 * @subpackage    cake.cake.libs.model.datasources.dbo
 */

class DboMssqlnative extends DboSource {
    ...
    * @var string
    */
    var $description = "MS SQL Native DBO Driver";
    ...
    function __construct($config, $autoConnect = true) {
        if ($autoConnect) {
            if (!function_exists('sqlsrv_num_rows')) {
                trigger_error("PHP SQL Server Native Driver is not installed or currently installed driver is not correct version (v1.1 required). To download the driver please see http://www.microsoft.com/downloads/details.aspx?FamilyID=ccdf728b-1ea0-48a8-a84a-5052214caad9&displaylang=en", E_USER_WARNING);
            }
        }
    ...
    */
    function connect() {
        ...
        $this->connection = sqlsrv_connect($config['host'], array('Database' => $config['database'], 'UID' => $config['login'], 'PWD' => $config['password']));
    ...
    * @access protected
    */
    function _execute($sql, $scrollable = true) {
        $this->lastStatement = $sql; 
        if($scrollable){
            $result = sqlsrv_query($this->connection, $sql, array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET )) 
        } else {
    ...
    */
    function lastAffected() {
        return ($this->result) ? sqlsrv_rows_affected($this->result) : NULL;
    }
    ...
    */
    function lastNumRows() {
        return ($this->_result) ? sqlsrv_num_rows($this->_result) : false;
    }
    ...
    */
    function resultSet(&$results) {
        ...
        $this->map = array();
        $numFields = sqlsrv_num_fields($this->results);
        $index = 0;
        $j = 0;
        
        while ($j < $numFields) {
        $fieldData = sqlsrv_field_metadata($results);
        $column = $fieldData[$j]['Name'];
            if (strpos($column, '__')) {
    ...
    */
    function fetchResult() {
        $row = sqlsrv_fetch_array($this->results, SQLSRV_FETCH_ASSOC);
        if($row !== false){
            $resultRow = array();
            $i = 0;
            foreach ($row as $index => $field) {
                list($table, $column) = $this->map[$i];
                $resultRow[$table][$column] = $field;
                $i++;
            }
            return $resultRow;            
        } else {
